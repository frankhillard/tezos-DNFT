...
let () = Log.describe("[Collection.authorize] test suite")
...
let test_success =
    ...
    Collection_helper.assert_user_in_whitelist(collec.taddr, other, true)
...
let test_success_multiple =
    ...
    Collection_helper.assert_user_in_whitelist(collec.taddr, alice, true)

(* Failing authorize because not admin *)
let test_failure_only_admin =
    let (accounts, collec) = bootstrap() in
    let (_admin, other, _, _, _) = accounts in
    // let () = Collection_helper.with_preminted(admin, collec.contr,admin) in
    let () = Test.set_source other in
    let r = Collection_helper.authorize([other], 0tez, collec.contr) in
    Assert.string_failure r Collection.Errors.only_admin

(* Failing authorize because amount not null *)
let test_failure_with_amount_not_null =
    let (accounts, collec) = bootstrap() in
    let (admin, other, _, _, _) = accounts in
    // let () = Collection_helper.with_preminted(admin, collec.contr,admin) in
    let () = Test.set_source admin in
    let r = Collection_helper.authorize([other], 10tez, collec.contr) in
